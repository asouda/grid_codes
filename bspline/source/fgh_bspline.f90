...
!! using the b-spline for the potentials
!!
!!------------------------------------------------------------------
!!
!! $Author: souda $
...
!!------------------------------------------------------------------
   ...
   real(8) :: reactant_pot_min, product_pot_min, xt, bt
   ...
   real(8), allocatable, dimension(:,:) :: reactant_wavef, product_wavef, overlap, alpha
   ...
   if (narg.eq.0) then
      ...
      write(*,*) "================================================================================================"
      write(*,*) " Usage:"
      write(*,*) " fgh_bspline.bin <name_of_the_file_with_the_potential> <n_grid_points> <particle_mass> <nstates>"
      write(*,*) "================================================================================================"
      stop
   endif
...
11 continue
   ...
   reactant_pot_min = 9999.d0
   do i=2,n-1
      reactant_pot(i) = dbsval(x(i),korder,xknot,npoints,bscoef)
      if (reactant_pot(i).le.reactant_pot_min) reactant_pot_min = reactant_pot(i)
   enddo
   reactant_pot(n) = reactant_pot_data(npoints)
   reactant_pot = (reactant_pot - reactant_pot_min)*h2kcal
   ...
   product_pot_min = 9999.d0
   do i=2,n-1
      product_pot(i) = dbsval(x(i),korder,xknot,npoints,bscoef)
      if (product_pot(i).le.product_pot_min) product_pot_min = product_pot(i)
   enddo
   product_pot(n) = product_pot_data(npoints)
   product_pot = (product_pot - product_pot_min)*h2kcal
   ...
   do i=1,npoints
      write(1,'(3g20.10)') x_data(i),&
      &(reactant_pot_data(i)-reactant_pot_min)*h2kcal,&
      &(product_pot_data(i)-product_pot_min)*h2kcal
   enddo
   close(1)
